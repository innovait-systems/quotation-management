...
async function main() {
  ...
  // 2. Create Tenant Quota allocations
  ...
  // 3. Create Default Users (Encrypted Password)
  const salt = await bcrypt.genSalt(10);
  const passwordHash = await bcrypt.hash('AdminSecure2026!', salt);
  ...
  // 4. Seed Dynamic Custom Fields (Build Metadata FIRST)
  // Field 1: Warranty Months for Quotes
  ...
  // Field 2: Custom Client SLA Rating for Service tracking
  ...
  // Field 3: Service KT Record verification checkbox
  ...
  console.log('✅ Dynamic metadata fields initialized.');

  // 5. Seed 3-Layer Document Templates
  const standardInvoiceTemplate = await prisma.template.create({
    data: {
      ...
      // Layer 1: Expected properties mapping
      dataSchema: {
        properties: {
      ...
      // Layer 2: Grid and visual positions
      layoutConfig: {
        pageSize: 'A4',
      ...
      // Layer 3: Branding typography & colors
      themeConfig: {
        fontFamily: 'Outfit',
        primaryHex: '#6366f1',
        secondaryHex: '#0f172a',
  ...
  console.log(`✅ Standard templates seeded: ${standardInvoiceTemplate.name}`);

  // 6. Seed Operational Approval Templates
        ...
        create: [
          {
            ...
            escalationTimeout: 1440, // 24 hours
            escalationRules: { action: 'ALERT_ADMIN' }
          },
          {
            stepSequence: 2,
            assignedUserId: adminUser.id,
            escalationTimeout: 2880, // 48 hours
  ...
  console.log(`✅ Approval matrices seeded: ${highValueApproval.name}`);

  // 7. Seed Base Automation Rules
  ...
  console.log('✅ Baseline operational automation rules seeded.');
